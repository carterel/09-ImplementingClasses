...
class Point(object):
    ...
    def closer_to(self, p2, p3):
        ...
        distance_from_p2 = math.sqrt(((p2.x - self.x) ** 2) + ((p2.y - self.y) ** 2))
        distance_from_p3 = math.sqrt(((p3.x - self.x) ** 2) + ((p3.y - self.y) ** 2))

        if distance_from_p2 < distance_from_p3:
            return p2

        elif distance_from_p3 < distance_from_p2:
            return p3

        elif distance_from_p2 == distance_from_p3:
            return p2

    def halfway_to(self, p2):

        halfway_x = self.previous_x + (p2.x - self.previous_x) / 2
        halfway_y = self.previous_y + (p2.y - self.previous_y) / 2
